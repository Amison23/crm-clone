...
async function main() {
  console.log("Sending query to Supabase SQL endpoint...");
  
  // Try sending query to Supabase's SQL endpoint / query API
  ...
  if (!res.ok) {
    console.log("REST /query response status:", res.status, await res.text());
  } else {
    console.log("SQL executed successfully via /query!", await res.json());
  }
}
